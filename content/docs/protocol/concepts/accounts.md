...
Cosmos blockchains, like TSP, support creating accounts with mnemonic phrases,
otherwise known as [hierarchical deterministic key generation](https://github.com/confio/cosmos-hd-key-derivation-spec) (HD keys). This allows the user to create accounts on multiple blockchains without having to manage multiple secrets.
...
The PubKey can be derived to generate various addresses in different formats, which are used to identify users (among other parties) in the application. A common address form for Cosmos chains is the bech32 format (e.g. `tsp1...`). Addresses are also associated with messages to identify the sender of the message.
...
TSP defines its own custom `Account` type to implement a HD wallet that is compatible with Ethereum type addresses. It uses Ethereum's ECDSA secp256k1 curve for keys (`eth_secp265k1`) and satisfies the [EIP84](https://github.com/ethereum/EIPs/issues/84) for full [BIP44](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki) paths. This cryptographic curve is not to be confused with [Bitcoin's ECDSA secp256k1](https://en.bitcoin.it/wiki/Secp256k1) curve.

The root HD path for TSP-based accounts is `m/44'/60'/0'/0`. TSP uses the Coin type `60` to support Ethereum type accounts, unlike many other Cosmos chains that use Coin type `118` ([list of coin types](https://github.com/satoshilabs/slips/blob/master/slip-0044.md)

The custom TSP [EthAccount](https://github.com/evmos/evmos/blob/main/types/account.go#L28-L33) satisfies the `AccountI` interface from the Cosmos SDK auth module and includes additional fields that are required for Ethereum type addresses:
...
For more information on Ethereum accounts head over to the [x/evm module](/docs/protocol/modules/evm#concepts).

### Addresses and Public Keys

[BIP-0173](https://github.com/satoshilabs/slips/blob/master/slip-0173.md) defines a new format for segregated witness
output addresses that contains a human-readable part that identifies the Bech32 usage. TSP uses the following
...
There are 3 main types of HRP for the `Addresses`/`PubKeys` available by default on TSP:
...
| --- | --- | --- | --- | --- | --- |
| Accounts | `tsp` | `tsppub` | `eth_secp256k1` | `20` | `33` (compressed) |
| Validator Operator | `tspvaloper` | `tspvaloperpub` | `eth_secp256k1` | `20` | `33` (compressed) |
| Consensus Nodes | `tspvalcons` | `tspvalconspub` | `ed25519` | `20` | `32` |

### Address formats for clients

`EthAccount` can be represented in both [Bech32](https://en.bitcoin.it/wiki/Bech32) (`tsp1...`)
...
- **Address (Bech32)**: `tsp1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw`
...
The `tspd debug addr <address>` can be used to convert an address between hex and bech32 formats. For example:

Bech32
```
 $ tspd debug addr evmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
...
Hex
```
 $ tspd debug addr 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Address: [20 87 74 109 255 45 223 158 7 130 139 67 69 211 4 9 25 175 86 82]
  Address (hex): 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Bech32 Acc: tsp1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
  Bech32 Val: tsploper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn
```

### Key output

{{< callout type="info" >}}
The Cosmos SDK Keyring output (i.e `tspd keys`) only supports addresses and public keys in Bech32 format.
{{< /callout >}}
...
**Accounts**
```
 $ tspd keys show dev0 --bech acc
- name: dev0
  type: local
  address: tsp1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
...
**Validator**
```
 $ tspd keys show dev0 --bech val
- name: dev0
  type: local
  address: tspvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn
  pubkey: '{"@type":"/ethermint.crypto.v1.ethsecp256k1.PubKey","key":"AsV5oddeB+hkByIJo/4lZiVUgXTzNfBPKC73cZ4K1YD2"}'
  mnemonic: ""
```
...
**Consensus**
```
 $ tspd keys show dev0 --bech cons
- name: dev0
  type: local
  address: tspvalcons1rllqa5d97n6zyjhy6cnscc7zu30zjn3f7wyj2n
...
tspd q auth account $(tspd keys show dev0 -a) -o text
...
use the JSON-RPC [`eth_accounts`](/docs/develop/api/ethereum-json-rpc/methods#eth-accounts) or [`personal_listAccounts`](/docs/develop/api/ethereum-json-rpc/methods#personal-listAccounts) endpoints:
